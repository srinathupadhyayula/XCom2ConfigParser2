...
public class AssetCooker
{
        ...
        CancellationToken ct)
    {
        ...
        var cookStep = new ModAssetsCookStep(buildOptions, contentOptions, stagingPath, _runner, _mirror, _tracker, _loggerFactory, _loggerFactory.CreateLogger<ModAssetsCookStep>());
    ...
    public virtual async Task CleanAsync(string modName, CancellationToken ct)
    {
        ...
        // Remove from XComGame/Published/CookedPCConsole/[ModName]
        var publishedPath = Path.Combine(_sdkPath, "XComGame", "Published", "CookedPCConsole", modName);
        if (Directory.Exists(publishedPath))
        {
            _logger.LogDebug($"Deleting published cooked assets: {publishedPath}");
            Directory.Delete(publishedPath, true);
        }

        // Remove .tfc files from XComGame/Content/Mods/[ModName]
        var tfcPath = Path.Combine(_sdkPath, "XComGame", "Content", "Mods", modName);
        if (Directory.Exists(tfcPath))
        {
            var tfcFiles = Directory.GetFiles(tfcPath, "*.tfc");
            foreach (var file in tfcFiles)
            {
                _logger.LogDebug($"Deleting TFC file: {file}");
                File.Delete(file);
            }
        }
