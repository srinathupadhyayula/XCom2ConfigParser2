...
/// <summary>
/// Passes all output directly to console with immediate flush for live streaming.
/// </summary>
...
public class ModcookReceiver : OutputReceiver
{
    ...
    public override void ParseLine(string? line)
    {
        ...
        if (line.StartsWith("GFx movie package", StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                Console.WriteLine("[GFx movie packages ...]");
            }
...
public class MakeOutputReceiver : OutputReceiver
{
    ...
    public MakeOutputReceiver(string[] reversePaths)
    {
    ...
    private void WriteColoredOutput(string line, bool isError)
    {
        ...
        if (!line.Contains("Warning/Error Summary", StringComparison.OrdinalIgnoreCase) && 
            line.Contains("Warning", StringComparison.OrdinalIgnoreCase) || line.Contains("Error", StringComparison.OrdinalIgnoreCase))
        {
            if (summaryMatch.Success)
            {
                var numErr = int.Parse(summaryMatch.Groups[2].Value);
                var numWarn = int.Parse(summaryMatch.Groups[3].Value);
