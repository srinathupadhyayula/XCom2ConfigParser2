...
public readonly struct Token
{
    ...
    /// <summary>
    /// Gets the character range in the source text where this token's content is located.
    /// </summary>
    public Range Range { get; }

    /// <summary>
    /// Gets the absolute character position in the source text where this token begins.
    /// </summary>
    public int Position { get; }
    ...
    /// <param name="range">The character range in the source.</param>
    /// <param name="position">The source position.</param>
    public Token(TokenType type, Range range, int position)
    {
        Type = type;
        Range = range;
        Position = position;
    ...
    public static Token LParen(int pos) => new(TokenType.LParen, new Range(pos, pos + 1), pos);
    /// <summary>Creates a closing parenthesis token.</summary>
    public static Token RParen(int pos) => new(TokenType.RParen, new Range(pos, pos + 1), pos);
    /// <summary>Creates an opening square bracket token.</summary>
    public static Token LBrack(int pos) => new(TokenType.LBrack, new Range(pos, pos + 1), pos);
    /// <summary>Creates a closing square bracket token.</summary>
    public static Token RBrack(int pos) => new(TokenType.RBrack, new Range(pos, pos + 1), pos);
    /// <summary>Creates a comma delimiter token.</summary>
    public static Token Comma(int pos) => new(TokenType.Comma, new Range(pos, pos + 1), pos);
    /// <summary>Creates an equals sign assignment token.</summary>
    public static Token Eq(int pos) => new(TokenType.Eq, new Range(pos, pos + 1), pos);
    /// <summary>Creates a semicolon comment token.</summary>
    public static Token Semi(int pos) => new(TokenType.Semi, new Range(pos, pos + 1), pos);
    /// <summary>Creates an unquoted text value token.</summary>
    public static Token TextValue(Range range, int pos) => new(TokenType.Text, range, pos);
    /// <summary>Creates a double-quoted string token.</summary>
    public static Token Quoted(Range range, int pos) => new(TokenType.Quoted, range, pos);
    /// <summary>Creates an end-of-file token.</summary>
    public static Token Eof(int pos) => new(TokenType.Eof, new Range(pos, pos), pos);
}
