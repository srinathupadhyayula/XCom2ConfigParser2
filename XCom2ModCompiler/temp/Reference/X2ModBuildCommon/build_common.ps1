...
class BuildProject {
	...
	BuildProject(
		...
		$this.finalModPath = "$modDestinationPath\$($this.modNameCanonical)"
	}
	...
	[void]InvokeBuild() {
		try {
			...
			if ($this._HasScriptPackages()) {
				$this._PerformStep({ ($_)._CleanLeftoverScripts() }, "Cleaning", "Cleaned", "leftover script packages from SDK/Game directory")
			}
	...
	[void]_SetupUtils() {
		...
		# finalModPath is set in constructor to use modDestinationPath parameter
		...
		# Write manifest of all compiled packages
		$manifestPath = [io.path]::combine($global:buildCommonSelfPath, 'CompiledModPackages.txt')
		$this._GetAllScriptPackages() | Set-Content $manifestPath
	}

	[string[]]_GetAllScriptPackages() {
		$pkgs = @()
		if ($this._HasScriptPackages()) {
			$pkgs += $this.modScriptPackages
		}
		foreach ($includeDir in $this.include) {
			if (Test-Path $includeDir) {
				$pkgs += Get-ChildItem $includeDir -Directory | Select-Object -ExpandProperty Name
			}
		}

		# Filter out native packages that we shouldn't touch
		$filteredPkgs = $pkgs | Where-Object { $global:nativescriptpackages -notcontains $_ } | Select-Object -Unique
		return $filteredPkgs
	}
	...
	[void]_RegenerateItemGroup() {
		try {
			# Load the .x2proj file as XML
			[xml]$x2projXml = Get-Content -Path $this.modX2projPath

			# Get the project node
			$projectNode = $x2projXml.Project

			# Remove existing ItemGroups
			Write-Host "Clearing old ItemGroups..."
			$existingItemGroups = $projectNode.ChildNodes | Where-Object { $_.Name -eq "ItemGroup" }
			foreach ($itemGroup in $existingItemGroups) {
            	foreach ($child in $itemGroup.ChildNodes) {
                	if ($child.Name -eq "Folder" -or $child.Name -eq "Content") {
                    	$includeValue = $child.Attributes["Include"].Value
                    	Write-Host "Removed $($child.Name) '$includeValue' from ItemGroups"
                	}
            	}
            [void]$projectNode.RemoveChild($itemGroup)
        	}

			Write-Host "Scanning project directory for files and folders..."

			# Enumerate all files recursively, excluding the .x2proj file itself
			$allFiles = Get-ChildItem -Path $this.modSrcRoot -Recurse -File |
				Where-Object { $_.FullName -ne $this.modX2projPath } |
				ForEach-Object {
					# Convert to relative path
					$relativePath = $_.FullName.Replace($this.modSrcRoot, '').TrimStart('\').Replace('/', '\')
					$relativePath
				}

			# Build list of unique folder paths from the files
			$allFolders = $allFiles |
				ForEach-Object {
					$pathComponents = $_.Split('\')
					$folders = @()
					for ($i = 0; $i -lt ($pathComponents.Length - 1); $i++) {
						$folderPath = $pathComponents[0..$i] -join '\'
						$folders += $folderPath
					}
					$folders
				} |
			...
			$newItemGroup = $x2projXml.CreateElement("ItemGroup", $projectNode.NamespaceURI)
			[void]$projectNode.AppendChild($newItemGroup)
			...
			# Add Folder elements
			foreach ($folder in ($allFolders | Sort-Object)) {
				$folderNode = $x2projXml.CreateElement("Folder", $projectNode.NamespaceURI)
				$includeAttr = $x2projXml.CreateAttribute("Include")
				$includeAttr.Value = $folder
				[void]$folderNode.Attributes.Append($includeAttr)
				$allNodes += $folderNode
			}

			# Add Content elements
			foreach ($file in ($allFiles | Sort-Object)) {
				$contentNode = $x2projXml.CreateElement("Content", $projectNode.NamespaceURI)
				$includeAttr = $x2projXml.CreateAttribute("Include")
				$includeAttr.Value = $file
				[void]$contentNode.Attributes.Append($includeAttr)
				$allNodes += $contentNode
			}

			# Sort nodes first by Name ascending (Folder before Content), then by Include ascending
			$sortedNodes = $allNodes | Sort-Object -Property { $_.Name }, { ($_.Attributes["Include"].Value -split '\\').Count }, { $_.Attributes["Include"].Value }
			foreach ($node in $sortedNodes) {
				Write-Host "Added $($node.Name) '$($node.Attributes["Include"].Value)' to ItemGroups"
				[void]$newItemGroup.AppendChild($node)
			}

			# Save the updated XML
			$x2projXml.Save($this.modX2projPath)

			Write-Host "ItemGroup regeneration completed successfully for project '$($this.modNameFull)'."
		} catch {
			Write-Host "Warning: ItemGroup regeneration failed. Error: $($_.Exception.Message)"
		}
	}
	...
	[void]_CheckCleanCompiled() {
		...
		if ($rebuild) {
			Write-Host "Selective cleaning of compiled scripts from $($this.sdkPath)/XComGame/Script to avoid compiler error..."
	...
	[void]_RunMakeBase() {
		...
		$this._InvokeEditorCmdlet($handler, $scriptsMakeArguments, 1000, 2000)
		...
		if ($this.final_release -eq $true) {
			...
			$handler = [MakeStdoutReceiver]::new($this)
			$handler.processDescr = "compiling base game scripts"
			$this._InvokeEditorCmdlet($handler, $scriptsMakeArguments, 1000, 20000)
		}
	}
	...
	[void]_RunMakeMod() {
		...
		$this._InvokeEditorCmdlet($handler, $scriptsMakeArguments, 1000, 1000)
	}
	...
	[void]_CopyScriptPackages() {
		...
		foreach ($name in $this.modScriptPackages) {
			...
			{
				# Or this is a non-native package - check if it exists first
				$packagePath = "$($this.sdkPath)\XComGame\Script\$name.u"
				if (Test-Path $packagePath) {
					Copy-Item $packagePath "$($this.stagingPath)\Script" -Force -WarningAction SilentlyContinue
					Write-Host $packagePath
				} else {
					Write-Host "Package $name.u not found - likely merged into another package due to dependencies"
				}
			}
		}
	}
	...
	[void]_PrecompileShaders() {
		...
		if ($need_shader_precompile)
		{
			...
			$this._InvokeEditorCmdlet($handler, $precompileShadersFlags, 500, 500)
	...
	[void]_FinalCopy() {
		# Move all staged files to the actual game's mods folder (copy then delete for safety)

		# First, ensure destination directory exists
		if (!(Test-Path "$($this.finalModPath)")) {
			New-Item -ItemType Directory -Path "$($this.finalModPath)" -Force
		}

		# Copy all files from staging to final location
		$robocopyResult = Robocopy.exe "$($this.stagingPath)" "$($this.finalModPath)" *.* $global:def_robocopy_args

		# Check if the copy was successful before removing source
		$exitCode = $LASTEXITCODE
		if ($exitCode -le 7) {  # robocopy exit codes: 0-7 generally indicate success (with varying degrees of changes)
			# Verify the destination was properly created before deleting source
			if (Test-Path "$($this.finalModPath)") {
				# Get the number of items in source and destination to compare
				$sourceItems = (Get-ChildItem -Path "$($this.stagingPath)" -Recurse -Force | Measure-Object).Count
				$destItems = (Get-ChildItem -Path "$($this.finalModPath)" -Recurse -Force | Measure-Object).Count

				# If destination has items (at least as many as source), we can safely remove source
				if ($destItems -gt 0) {
					Write-Host "Removing source staging directory $($this.stagingPath) after successful move to $($this.finalModPath)..."
					Remove-Item -Recurse -Force "$($this.stagingPath)"
					Write-Host "Source directory removed. Move operation completed successfully."
				} else {
					Write-Warning "Destination directory exists but has no items. Source directory preserved for safety."
				}
			} else {
				Write-Warning "Destination directory was not created. Source directory preserved for safety."
			}
		} else {
			Write-Warning "Robocopy failed with exit code $exitCode. Source directory preserved for safety."
			Write-Warning "Error details: $robocopyResult"
		}
	}
		...
		foreach ($name in $pkgs) {
			...
			foreach ($path in @($sdkScriptPath, $sdkFinalScriptPath, $gameScriptPath)) {
				if (Test-Path $path) {
					...
					Remove-Item -Force $path -WarningAction SilentlyContinue -ErrorAction SilentlyContinue
				}
			}
		}
	}
	...
	[void]_InvokeEditorCmdlet([StdoutReceiver] $receiver, [string] $makeFlags, [int] $sleepAtStartMs = 0, [int] $sleepAtEndMs = 0) {
		...
		if ($sleepAtEndMs -gt 0) {
			Write-Host "Waiting for $($sleepAtEndMs / 1000.0) seconds..."
			Start-Sleep -Milliseconds $sleepAtEndMs
		}
	}
...
class ModAssetsCookStep {
	...
	[void] _InvokeAssetCooker ([string] $editorArguments) {
		...
		$this.project._InvokeEditorCmdlet($handler, $editorArguments, 0, 0)
	}
...
class MakeStdoutReceiver : StdoutReceiver {
	...
	MakeStdoutReceiver([BuildProject]$proj) {
		$this.proj = $proj
