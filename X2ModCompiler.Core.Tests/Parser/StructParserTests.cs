...
public class StructParserTests
{
    ...
    [Fact]
    public void Parse_StructWithNumericIndex_ReturnsIndexedProperty()
    {
        var result = StructParser.Parse("(Prop[0]=Value)");

        var structValue = result.ShouldBeOfType<StructValue>();
        structValue.Children[0].Name.ShouldBe("Prop");
        structValue.Children[0].Index.ShouldBe("0");
    }

    [Fact]
    public void Parse_StructWithEnumIndex_ReturnsIndexedProperty()
    {
        var result = StructParser.Parse("(Prop[eStat_Will]=Value)");

        var structValue = result.ShouldBeOfType<StructValue>();
        structValue.Children[0].Name.ShouldBe("Prop");
        structValue.Children[0].Index.ShouldBe("eStat_Will");
    }
