...
/// </summary>
public class BuildControllerSelectiveCleanTests : IDisposable
{
    ...
    public async Task InvokeBuildAsync_PerformsSelectiveClean_WhenBuildModeChanges()
    {
        ...
        var oldFingerprint = new BuildFingerprint("debug", "hash123", DateTime.UtcNow, DateTime.UtcNow, new Dictionary<string, DateTime>());
        _trackerMock.Setup(x => x.LoadFingerprintAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync(oldFingerprint);
        _trackerMock.Setup(x => x.ShouldRebuildAsync(It.IsAny<BuildOptions>(), It.IsAny<string>(), It.IsAny<DateTime?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
        _trackerMock.Setup(x => x.GetSelectiveCleanPathsAsync(It.IsAny<string>(), It.IsAny<string[]>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<string> { "test.u" });
        ...
        _trackerMock.Verify(x => x.ShouldRebuildAsync(
            It.IsAny<BuildOptions>(),
            It.IsAny<string>(),
            It.IsAny<DateTime?>(),
    ...
    public async Task InvokeBuildAsync_DoesNotPerformSelectiveClean_WhenNothingChanged()
    {
        ...
        var currentFingerprint = new BuildFingerprint("debug", "hash123", DateTime.UtcNow, DateTime.UtcNow, new Dictionary<string, DateTime>());
        _trackerMock.Setup(x => x.LoadFingerprintAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync(currentFingerprint);
        _trackerMock.Setup(x => x.ShouldRebuildAsync(It.IsAny<BuildOptions>(), It.IsAny<string>(), It.IsAny<DateTime?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(false);
        ...
        _mirrorMock.Verify(m => m.DeleteAsync(
            It.IsAny<string>(),
            It.IsAny<bool>(),
            It.IsAny<CancellationToken>()),
            Times.Never);
    ...
    private BuildOptions CreateValidOptions()
    {
        ...
        {
            ModName = "TestMod",
            ProjectRoot = Path.Combine(_tempPath, "TestMod"),
